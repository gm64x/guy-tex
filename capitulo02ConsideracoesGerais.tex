...
A ferramenta proposta nesta pesquisa adota uma arquitetura de extensão para o \textit{Visual Studio Code}. A lógica principal executa no \textit{Extension Host}, utilizando Node.js e TypeScript para acessar o arquivo aberto no editor, acionar o processo de análise estática e coordenar a comunicação com a interface. Para a camada visual, o framework \textbf{React.js} é utilizado em uma \textit{Webview}, permitindo a renderização interativa do Grafo de Fluxo de Controle \citeonline{react_docs}.

Para o processamento de código e análise sintática (\textit{parsing}), a arquitetura foi projetada visando a extensibilidade e o suporte a múltiplos paradigmas de programação, visto que a linguagem-alvo de análise ainda está em definição. Para isso, adota-se preferencialmente o \textbf{Tree-sitter} \citeonline{TreeSitterWeb}, uma ferramenta moderna de geração de analisadores que pode ser utilizada com \textit{WebAssembly} para executar com desempenho adequado no ambiente local da extensão. Como alternativa consolidada no meio acadêmico e na construção de compiladores, considera-se também a utilização do \textbf{ANTLR} (\textit{Another Tool for Language Recognition}) \cite{ANTLRWeb}.
